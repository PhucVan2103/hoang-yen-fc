...
const IPhone14Frame = ({ children }) => {
  return (
    <div className="md:flex md:items-center md:justify-center md:min-h-screen md:bg-slate-100 font-sans">
      {/* Container của Frame: Trên mobile là w-full h-screen, trên desktop là kích thước cố định */}
      <div className="relative w-full h-screen md:h-[844px] md:w-[390px] md:border-gray-800 md:dark:border-gray-800 md:bg-gray-800 md:border-[14px] md:rounded-[3.5rem] md:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.3)] overflow-hidden">
        ...
        {/* Antenna lines - Chỉ hiển thị trên Desktop */}
        ...
        {/* Dynamic Island - Chỉ hiển thị trên Desktop */}
        ...
        {/* Status Bar Giả lập - Chỉ hiển thị trên Desktop */}
        ...
        {/* Screen Content - Tràn viền trên mobile, bo góc trên desktop */}
        <div className="h-full w-full bg-[#f4f6f8] overflow-hidden md:rounded-[2.5rem] relative">
          {children}
        </div>

        {/* Home Indicator Giả lập - Chỉ hiển thị trên Desktop */}
...
export default function App() {
  ...
  const [isAdminView, setIsAdminView] = useState(true); // Chế độ hiển thị của Admin
  ...
  const handleShieldClick = () => {
    if (!isAdmin) {
      ...
      if (isAdminView) {
        if (window.confirm("Bạn muốn chuyển sang chế độ người xem? (Các công cụ quản lý sẽ tạm ẩn)")) {
          setIsAdminView(false);
        }
      } else {
        if (window.confirm("Bạn muốn quay lại chế độ quản lý?")) {
          setIsAdminView(true);
        }
      }
    }
  ...
  const handleLoginSubmit = (e) => {
    e.preventDefault();
    if (passwordInput === 'hoangyenfc@2026') {
      setIsAdmin(true);
      setIsAdminView(true);
  ...
  const handleDeleteData = async (id, type) => {
    if (!isAdmin || !isAdminView || !window.confirm("Bạn có chắc chắn muốn xóa mục này?")) return;
    try {
  ...
  return (
    <IPhone14Frame>
      <div className="flex flex-col h-full text-slate-900 select-none overflow-hidden relative">
        ...
        <header className="pt-14 pb-4 px-6 flex justify-between items-center bg-[#f4f6f8] shrink-0 z-10 relative">
          {currentScreen === 'home' ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold tracking-tight text-slate-900 leading-tight truncate max-w-[120px]">{activeTournamentName}</h1>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-sm ${
                isAdmin 
                ? (isAdminView ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600') 
                ...
                {tournaments.length === 0 ? (
                  ...
                  tournaments.map(t => (
                    <div 
                      ...
                      className="relative h-[330px] rounded-[1.75rem] overflow-hidden shadow-[0_8px_20px_-8px_rgba(0,0,0,0.15)] cursor-pointer group active:scale-[0.98] transition-transform bg-slate-200"
                    >
          ...
          <main className="flex-1 overflow-y-auto pb-32 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/5">
                      ...
                      <p className="font-bold text-sm tracking-wide">+{formatCurrency(stats.totalIncome + stats.totalDonations)}</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/5">
                      ...
                      <p className="font-bold text-sm tracking-wide">-{formatCurrency(stats.totalExpense)}</p>
                    </div>
                  </div>
                </div>
            ...
            <div className="px-5 mt-6 mb-4">
              <div className="flex bg-slate-200/70 p-1.5 rounded-2xl">
                <button onClick={() => setActiveTab('transactions')} className={`flex-1 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'transactions' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Giao Dịch</button>
                <button onClick={() => setActiveTab('donations')} className={`flex-1 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'donations' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Ủng Hộ</button>
              </div>
            </div>
            ...
            <div className="px-5 space-y-3">
              {activeTab === 'transactions' ? (
                currentTransactions.length === 0 ? (
                  <div className="text-center py-10 text-slate-400"><p className="text-sm">Chưa có dữ liệu giao dịch.</p></div>
                ) : (
                  currentTransactions.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-[1.25rem] flex items-center gap-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.03)] border border-slate-100/50">
                      ...
                      <div className="text-right flex flex-col items-end gap-1">
                        <p className={`font-bold text-[15px] ${item.type === 'income' ? 'text-emerald-600' : 'text-slate-900'}`}>{item.type === 'income' ? '+' : '-'}{formatCurrency(item.amount)}</p>
                        {isAdmin && isAdminView && (
                          <button onClick={() => handleDeleteData(item.id, 'transaction')} className="text-slate-300 hover:text-rose-500 transition-colors p-1 -mr-1"><Trash2 size={15} /></button>
                        )}
                      </div>
                    </div>
                  ))
                ...
                currentDonations.length === 0 ? (
                  <div className="text-center py-10 text-slate-400"><p className="text-sm">Chưa có dữ liệu ủng hộ.</p></div>
                ) : (
                  currentDonations.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-[1.25rem] flex items-center gap-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.03)] border border-slate-100/50 relative overflow-hidden">
                      ...
                      <div className="shrink-0 w-12 h-12 bg-amber-50 text-amber-600 rounded-2xl flex items-center justify-center"><Heart size={20} strokeWidth={2.5} className="fill-amber-100" /></div>
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="text-right flex flex-col items-end gap-1">
                        ...
                        {isAdmin && isAdminView && (
                          <button onClick={() => handleDeleteData(item.id, 'donation')} className="text-slate-300 hover:text-rose-500 transition-colors p-1 -mr-1"><Trash2 size={15} /></button>
                        )}
                      </div>
                    </div>
                  ))
        ...
        {isAdmin && isAdminView && currentScreen === 'home' && (
          <button onClick={() => openTournamentModal()} className="absolute bottom-10 right-6 w-14 h-14 bg-slate-900 text-white rounded-2xl shadow-[0_10px_25px_-5px_rgba(15,23,42,0.5)] flex items-center justify-center z-30 active:scale-95 transition-transform"><Plus size={28} strokeWidth={2.5} /></button>
        )}
        
        {isAdmin && isAdminView && currentScreen === 'fund' && (
          <button onClick={() => setShowAddModal(true)} className="absolute bottom-10 right-6 w-14 h-14 bg-emerald-600 text-white rounded-2xl shadow-[0_10px_25px_-5px_rgba(5,150,105,0.4)] flex items-center justify-center z-30 active:scale-95 transition-transform"><Plus size={28} strokeWidth={2.5} /></button>
        )}
          ...
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-end">
            <div className="bg-white w-full rounded-t-[2rem] p-6 pt-3 shadow-2xl animate-in slide-in-from-bottom duration-300 pb-10">
              ...
              <div className="flex justify-between items-center mb-6 px-1">
                <h2 className="text-xl font-bold tracking-tight text-slate-900">{activeTab === 'transactions' ? 'Thêm Giao Dịch Mới' : 'Ghi Nhận Ủng Hộ'}</h2>
                <button onClick={() => setShowAddModal(false)} className="bg-slate-100 p-2 rounded-full text-slate-500 active:bg-slate-200 transition-colors"><X size={18} strokeWidth={2.5} /></button>
              </div>
              <form onSubmit={handleAddData} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 ml-1">{activeTab === 'donations' ? 'Tên Người Ủng Hộ' : 'Nội Dung Giao Dịch'}</label>
                  <input type="text" required placeholder="Nhập nội dung..." className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-[15px] font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all" value={activeTab === 'donations' ? formData.donorName : formData.note} onChange={(e) => activeTab === 'donations' ? setFormData({...formData, donorName: e.target.value}) : setFormData({...formData, note: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 ml-1">Số Tiền (VNĐ)</label>
                    <input type="number" required placeholder="0" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-[15px] font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 ml-1">Ngày Thực Hiện</label>
                    <input type="date" required className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-[15px] font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} />
                  </div>
                </div>
                {activeTab === 'donations' && (
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 ml-1">Lời Nhắn (Tùy chọn)</label>
                    <input type="text" placeholder="Ghi chú thêm..." className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-[15px] font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all" value={formData.note} onChange={(e) => setFormData({...formData, note: e.target.value})} />
                  </div>
                )}
                <button type="submit" className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold text-[15px] shadow-lg active:scale-[0.98] transition-transform mt-4">XÁC NHẬN LƯU</button>
              </form>
            </div>
          </div>
        )}
